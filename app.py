...
def generate_ticket(pool):
    ...
    if len(pool)<6: pool = list(range(1,50))
...
def generate_delta_tickets(draws, n_tickets=6):
    ...
    for _ in range(n_tickets):
        ...
        seq=[start]
        for _ in range(5):
            d=random.choice(common_deltas)
            next_num=seq[-1]+d
            if next_num<=49: seq.append(next_num)
...
def apply_exclusions_to_pool(pool, excluded):
...
def generate_repeat_ticket(last_draw, excluded, repeat_count=1):
...
with tab2:
    ...
    # Ensure excluded is a set for fast lookup
    excluded_set = set(excluded) if excluded else set()

    for _ in range(int(num_tickets)):
        # Generate a base ticket from delta analysis
        base_ticket = generate_delta_ticket(delta_counter)
        # Convert to plain Python ints
        base_ticket = [int(n) for n in base_ticket]

        # Apply exclusions to ticket pool
        pool = apply_exclusions_to_pool(base_ticket, excluded_set)
        pool = [int(n) for n in pool]

        # Fallback generator function
        def gen_ticket():
            if excluded_set and len(set(base_ticket) & excluded_set) > 0:
                return generate_ticket(pool)
            return base_ticket

        # Generate ticket respecting constraints
        ticket = try_generate_with_constraints(
            gen_ticket,
            sum_min=int(sum_min), sum_max=int(sum_max),
            spread_min=int(spread_min), spread_max=int(spread_max),
            odd_count=int(odd_count),
        )

        if ticket:
            # Ensure ticket numbers are ints
