...
def most_common_per_position(numbers_df: pd.DataFrame):
    return {col: tuple(map(int, Counter(numbers_df[col]).most_common(1)[0])) for col in numbers_df.columns}
...
def generate_balanced_ticket():
    ...
    while True:
        ...
        ticket.sort()
        odds = sum(n % 2 == 1 for n in ticket)
        total = sum(ticket)
...
# ----------------
def passes_constraints(ticket: list[int], sum_min, sum_max, spread_min, spread_max, odd_count: int | None):
    total, spread, odds = sum(ticket), max(ticket) - min(ticket), sum(n % 2 == 1 for n in ticket)
    if odd_count is not None and odds != odd_count:
        return False
    if not (sum_min <= total <= sum_max) or not (spread_min <= spread <= spread_max):
        return False
    return True
...
odd_mode = st.sidebar.selectbox("Odd/Even constraint", ["Any", "0", "1", "2", "3", "4", "5", "6"])
odd_count = None if odd_mode == "Any" else int(odd_mode)

# ======================
# Analytics
# ======================
st.subheader(f"📄 Analyzed Draws: {len(numbers_df)}")
